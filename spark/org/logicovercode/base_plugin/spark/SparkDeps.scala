package org.logicovercode.base_plugin.spark
...
trait SparkDeps {
  ...
  def sparkcore(version : String, packageConfiguration : Configuration = Provided): ModuleID = {
    "org.apache.spark" %% "spark-core" % version % packageConfiguration
  }

  def sparksql(version : String, packageConfiguration : Configuration = Provided): ModuleID = {
    "org.apache.spark" %% "spark-sql" % version % packageConfiguration
  }

  def sparkhive(version : String, packageConfiguration : Configuration = Provided): ModuleID = {
    "org.apache.spark" %% "spark-hive" % version % packageConfiguration
  }

  def sparkstreaming(version : String, packageConfiguration : Configuration = Provided): ModuleID = {
  ...
  def sparkIdeDependencies(clusterVariant : String) : Seq[ModuleID] = {
    ...
    (isInsideIde, clusterVariant) match {
      case (true, "2.1.0.cloudera4") =>
        ...
        Seq("org.apache.spark" %% "spark-launcher" % "2.1.0.cloudera4",
          "org.apache.spark" %% "spark-yarn" % "2.1.0.cloudera4",
          "org.apache.hadoop" % "hadoop-client" % "2.6.0-cdh5.13.0" excludeAll ExclusionRule(organization = "javax.servlet")
        )

      case (true, "2.3.1") =>
        println("inside ide, returning dependencies for 2.3.1")
        Seq("org.apache.spark" %% "spark-launcher" % clusterVariant,
      ...
      case (true, cv) =>
        println("inside ide, returning empty dependencies as, cluster variant >" + cv + "< not supported")
        Seq()
      case (false, _) =>
  ...
  private def isInsideIde : Boolean = {
    ...
    val sparkStreamingScope = sys.env.get("SPARK_STREAMING_SCOPE").getOrElse("compile")
